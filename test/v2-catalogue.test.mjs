...
test("implemented routes use explicit pages and are excluded from placeholders", async () => {
  ...
  assert.deepEqual(implemented.map((route) => route.id), [
    ...
    "workflow-state-laboratory",
    "foreman-model",
  ]);
  const placeholderIds = new Set(getCataloguePlaceholderRoutes().map((route) => route.id));
  assert.ok(implemented.every((route) => !placeholderIds.has(route.id)));
